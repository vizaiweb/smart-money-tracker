...
def fetch_rss_data(source_name, url, max_items=5):
    if not url:
        return []
...
def fetch_all_news():
    ...
    for source_name, source_info in NEWS_SOURCES.items():
        if not source_info.get("active", True):
            continue
        feed_content = fetch_rss_data(source_name, source_info["url"])
...
def extract_signals_from_news(news_items):
    signals = []
    signal_keywords = {
        "CPU需求上升": ["CPU", "處理器", "x86", "ARM", "算力"],
        "算力供不應求": ["算力緊張", "產能不足", "缺貨", "shortage"],
        "AI需求爆發": ["AI推理", "AI應用", "大模型", "generative"],
        "電網投資": ["grid", "輸配電", "transformer"],
        "核能": ["nuclear", "SMR"],
        "國防": ["defense", "Pentagon", "military"],
        "太空": ["space", "satellite", "SpaceX"],
        "GLP-1": ["GLP-1", "weight loss", "diabetes"],
        "降息": ["rate cut", "Fed", "interest rate"]
    }
